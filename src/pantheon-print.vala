...
public class CustomOperation : Gtk.PrintOperation {        
    ...
    public CustomOperation (string[] args, Gtk.Window main_window) {
        ...
        draw_page.connect ( (operation, print_context, page_num)=> {
            ...
            var draw_layout = print_context.create_pango_layout ();
            draw_layout.set_wrap (Pango.WrapMode.WORD_CHAR);
            draw_layout.set_text (content, -1);
            
            int layout_width, layout_height;
            layout.get_size (out layout_width, out layout_height);
            
            context.move_to (height - layout_height/Pango.SCALE, - 72 / 2.54);
